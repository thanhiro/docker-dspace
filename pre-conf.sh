#!/bin/bash
  ...
  mkdir /build
        chmod -R 770 /build
        cd /build
        wget http://sourceforge.net/projects/dspace/files/DSpace%20Stable/5.2/dspace-5.2-src-release.tar.gz
        tar -zxf dspace-5.2-src-release.tar.gz
        rm dspace-5.2-src-release.tar.gz
    
        cd /build/dspace-5.2-src-release
        mvn -U package
